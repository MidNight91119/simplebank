...
import (
	...
	"github.com/MidNight91119/simplebank/val"
...
func (server *Server) CreateUser(ctx context.Context, req *pb.CreateUserRequest) (*pb.CreateUserResponse, error) {
	...
	if err != nil {
		if pqErr, ok := err.(*pq.Error); ok {
			...
			case "unique_violation":
				return nil, status.Errorf(codes.AlreadyExists, "username alraedy exists: %s", err)
			}
		}
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to distribute task send verify email")
	}
